...
SELECT 
    ...
    FullName,
    fn_decrypt(Contact) AS Contact,
    fn_decrypt(Email) AS Email,
    fn_decrypt(AdditionalInfo) AS AdditionalInfo,
    StudentStatus,
    CreatedByUser,
    CreatedOn,
    LastModifiedBy,
...
SELECT 
    ...
    FullName,
    CASE 
        WHEN CURRENT_ROLE() = 'FacultyRole' THEN fn_MaskContact(Contact_Plain)
        ELSE fn_decrypt(Contact)
    END AS Contact,
    CASE 
        WHEN CURRENT_ROLE() = 'FacultyRole' THEN fn_MaskEmail(Email_Plain)
        ELSE fn_decrypt(Email)
    END AS Email,
